...
#+begin_src bnf
  pattern -> atomic-pattern
           | ['op'] qual-op-id atomic-pattern
           | qual-word-id atomic-pattern
           | atomic-pattern qual-op-id atomic-pattern
           | pattern ':' ty
           | 'op' ['prefix'] op-id [':' ty] 'as' pattern
...
to various parts of the value. Match patterns are used in exception
handlers, function values, and case expressions.

...
#+begin_src bnf
  ...
  q-word-id -> (word-id '.')* word-id
  q-operator-id -> (word-id '.')* operator-id
...
Emil permits two types of identifier. In many contexts where an
identifier is permitted, a qualified identifier is also permitted. A
qualified identifer is a dot-separated sequence of identifiers; all
but the last must be word identifiers, and the last may be a word or
operator identifer. Reserved words may not be used as identifiers.
